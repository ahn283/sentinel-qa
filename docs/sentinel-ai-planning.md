...
### 목표
- pilot-ai가 PRD를 읽고 테스트케이스를 생성하면, sentinel-ai가 이를 실행하고 결과를 반환
...
- **sentinel-ai**: LLM 없음. 테스트 실행, 결과 수집, 리포트 등 인프라 담당 (MCP 서버)

### 프로젝트 이름
- **`sentinel-ai`** (오픈소스 공개 예정)
- npm 충돌 없음 확인
...
pilot-ai (LLM 보유)
  ...
  └─ sentinel-ai MCP 서버 호출 (stdio)
       ↓
sentinel-ai (MCP 서버 — 테스트 인프라)
  ├─ 테스트케이스 저장 및 관리
  ├─ 플랫폼별 UI 자동화 실행
...
> sentinel-ai가 외부에서 실행하고 결과를 수집하기에도 가장 적합하다.
...
sentinel-ai의 **테스트 실행용** MCP 서버와는 목적이 다르지만, pilot-ai가 탐색적 테스트나 DOM 스냅샷 수집 시 함께 활용할 수 있다.
...
- `sentinel-ai`는 독립 레포로 분리
- 오픈소스 공개 전제로 설계 (Eodin 브랜드 종속 최소화)
- sentinel-ai는 LLM을 직접 호출하지 않음 (AI 로직은 pilot-ai 담당)
...
pilot-ai가 테스트 코드 생성 시 sentinel-ai의 `get_selectors` 툴로 조회하여 참조한다.
...
- pilot-ai가 sentinel-ai를 subprocess로 실행
...
### sentinel-ai가 노출하는 MCP 툴

```typescript
import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { z } from "zod";

const server = new McpServer({ name: "sentinel-ai", version: "1.0.0" });
...
pilot-ai가 생성한 코드
    ↓
save_tests()로 sentinel-ai에 전달
    ↓
sentinel-ai: 임시 디렉토리에 .spec.ts 파일 작성
    ↓
...
save_tests()로 sentinel-ai에 전달
    ↓
sentinel-ai: 임시 디렉토리에 .yaml 파일 작성
    ↓
...
> 이 섹션은 pilot-ai의 동작을 설명하며, sentinel-ai 구현 범위 밖이다.
> sentinel-ai는 생성된 결과물을 받아 저장하고 실행하는 역할만 한다.

### 2-pass 생성 파이프라인 (pilot-ai가 수행)

```
Pass 1: PRD → 테스트케이스 YAML (플랫폼 무관한 시나리오, 사람이 읽고 리뷰 가능)
Pass 2: YAML + sentinel-ai의 get_selectors() → 실행 가능한 Playwright/Maestro 코드
```
...
sentinel-ai는 LLM API 키가 필요 없다. 필요한 설정은 테스트 실행 환경 관련뿐이다.
...
| 컴포넌트 | 형태 | 비고 |
|---------|------|------|
| sentinel-ai | npm 모노레포 (npm workspaces + turborepo) | MCP 서버로 동작 |
